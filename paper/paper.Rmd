...
```{r packages-latex, include = FALSE, eval = which_output("latex")}
library(kableExtra)
```
...
```{r scripts, include = FALSE, eval = TRUE}
temp <- new.env()
# figures
...
The two models provide similar results with a different parameters interpretation. In the next sections we will illustrate the differences and simulation strategies. Figure \@ref(fig:fig-logit-vs-probit) depicts the two distributions while Table L'output è `r if(which_output("latex")) "\\@ref(tab:tab-model-summary-latex)" else "\\@ref(tab:tab-model-summary-word)"` summarise the presented cumulative models with the proposed link function and the corresponding R code.
...
pp <- ss[, 4:7]/50
ss <- round(ss, 2)
ss$Y1 <- sprintf("%s (p = %s)", fmt(ss$Y1, "bold", "latex"), fmt(pp$Y1, "bold", "latex"))
ss$Y2 <- sprintf("%s (p = %s)", fmt(ss$Y2, "bold", "latex"), fmt(pp$Y2, "bold", "latex"))
ss$Y3 <- sprintf("%s (p = %s)", fmt(ss$Y3, "bold", "latex"), fmt(pp$Y3, "bold", "latex"))
ss$Y4 <- sprintf("%s (p = %s)", fmt(ss$Y4, "bold", "latex"), fmt(pp$Y4, "bold", "latex"))

ss$group <- rownames(ss)
ss <- select(ss, group, everything(), -tot)
dat <- rtabs$data_example_table$dat

ss <- map(rtabs$data_example_table$table, ~gsub("\\**", "", .x)) |> 
  bind_rows()
...
group_a <- apply(cum_p[cum_p$x == "a", -1], 2, mean)
group_b <- apply(cum_p[cum_p$x == "b", -1], 2, mean)
# now odds are almost the same and equal to the simulated b1 = ~1.09
odds(group_a) / odds(group_b)
log(odds(group_a) / odds(group_b))
```
...
cat_latent_plot(m = c(0, 1), s = 1, probs = rep(1/4, 4), link = "logit", plot = "both")
```

```{r fig-example-num-latent, fig.cap=filor::get_caption(), fig.place = TRUE}
num_latent_plot(x = rnorm(100), b1 = 1, probs = c(0.6, 0.2, 0.1, 0.1), link = "probit") +
  mtheme() +
  viridis::scale_color_viridis(discrete = TRUE)
```
...
(ref:fig-example-cat-latent) Example of the `cat_latent_plot()` function depicting the effect of a categorical predictor. On the left the shift in the latent mean and on the right the impact on the expected probabilities. The plot can be created using `cat_latent_plot(m = c(0, 0.5), s = 1, probs = rep(1/4, 4), link = "logit", plot = "both")`.

(ref:fig-example-num-latent) Example of the `num_latent_plot()` function depicting the effect of a continuous predictor on the expected probabilities. $\beta_1$ is the regression coefficient. The plot can be created using `num_latent_plot(x = runif(100), b1 = 1, probs = c(0.6, 0.2, 0.1, 0.1), link = "probit")`
...
(ref:tab-model-summary-latex) (ref:tab-model-summary-docx)
...
(ref:tab-dataset-example-latex) (ref:tab-dataset-example-docx)